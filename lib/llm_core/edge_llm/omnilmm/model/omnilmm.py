...
class OmniLMMModel(MistralModel):
    ...
    def __init__(
        ...
        super(OmniLMMConfig, self).__init__(config)
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            # HACK: for FSDP (Fully Sharded Data Parallel)
    ...
    def initialize_vision_modules(
        ...
        if not hasattr(self, "vision_tower"):
            state_dict = torch.load("/tt/data/public/multimodal/multimodal_model_ckpts/timm/eva02_enormous_patch14_clip_224.laion2b_plus.pt")
    ...
    def get_vision_embedding(self, pixel_values):
        if isinstance(self.vision_tower, list):
            vision_tower = self.vision_tower[0]  # HACK: for FSDP (Fully Sharded Data Parallel)
        else:
            vision_tower = self.vision_tower
    ...
    def get_vllm_embedding(self, data):
        ...
        for cur_input_ids, cur_input_embeds in zip(data['input_ids'], inputs_embeds):
            if (cur_input_ids == self.vision_config.im_patch_token).sum() == 0:
                ...
                # TODO: Find dummy_image_features, and fix later
                cur_input_embeds = cur_input_embeds + (0. * dummy_image_features).sum()  # type: ignore
                            ...
                            if orig_embeds_params is not None:
                                ...
                                cur_new_input_embeds = torch.cat(
                                    (
                                        ...
                                        cur_image_features, cur_input_embeds[image_start_token_pos + num_patches + 1:]
                                    ), dim=0)
                            cur_image_idx += 1
                        new_input_embeds.append(cur_new_input_embeds)
                    else:
